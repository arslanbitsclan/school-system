...
class Inquiry extends Admin_Controller {
    ...
    public function index() {
        redirect(base_url('inquiry/view'));
    }

    /* student form validation rules */

    protected function inquiry_validation() {
//        $getBranch = $this->getBranchDetails();
        ...
        $this->form_validation->set_rules('inq_type', translate('inquiry_type'), 'required');
        $this->form_validation->set_rules('inquiry_no', translate('inquiry_no'), 'trim|required|callback_unique_registerid');
        $this->form_validation->set_rules('visited_date', translate('visited_date'), 'trim|required');
    ...
    public function add() {
        ...
        if (isset($_POST['save'])) {
            $this->inquiry_validation();
            if ($this->form_validation->run() == true) {
                $post = $this->input->post();
                //save all student information in the database file
                $inquiryID = $this->inquiry_model->save($post, $getBranch);
        ...
        $this->data['sub_page'] = 'inquiry/add';
        $this->data['main_menu'] = 'inquiry';
        $this->data['inquiry_no'] = $this->inquiry_model->regSer();


//        echo '<pre>';
//        print_r($this->data['register_id']);
//        exit;

        $this->data['title'] = translate('create_inquiry');
    ...
    public function csv_import() {
                ...
                if ($csv_array) {
                    ...
                    foreach ($csv_array as $row) {
                        ...
                        if (count($csv_chk) <= 0) {
                            ...
                            redirect(base_url("inquiry/csv_import"));
                        }
                    }
                    if ($err_msg != null) {
        ...
        $this->data['branch_id'] = $branchID;
        $this->data['sub_page'] = 'inquiry/multi_add';
    ...
    public function disable_authentication() {
        ...
        if (isset($_POST['auth'])) {
            ...
            redirect(base_url('inquiry/disable_authentication'));
        }
        $this->data['branch_id'] = $branchID;
        $this->data['title'] = translate('deactivate_account');
        $this->data['sub_page'] = 'inquiry/disable_authentication';
    ...
    public function category() {
        if (isset($_POST['category'])) {
            ...
            if ($this->form_validation->run() !== false) {
                ...
                set_alert('success', translate('information_has_been_saved_successfully'));
                redirect(base_url('inquiry/category'));
            }
        }
        $this->data['title'] = translate('student') . " " . translate('details');
        $this->data['sub_page'] = 'inquiry/category';
        $this->data['main_menu'] = 'admission';
        $this->load->view('layout/index', $this->data);
    }
    ...
    public function view() {
        ...
        if (isset($_POST['search'])) {
            ...
            $this->data['inquiries'] = $inquiries = $this->application_model->getInquiries($classID, $sectionID, $branch_id, false, true);
        ...
        $this->data['main_menu'] = 'inquiry';
        $this->data['sub_page'] = 'inquiry/view';
        $this->data['headerelements'] = array(
            'js' => array(
                'js/student.js'
            ),
    ...
    public function profile($id = '') {
        ...
        if (isset($_POST['update'])) {
            ...
            if ($this->form_validation->run() == true) {
                ...
                set_alert('success', translate('information_has_been_updated_successfully'));
                redirect(base_url('inquiry/profile/' . $id));
            }
        }
        $this->data['student'] = $getStudent;
        $this->data['title'] = translate('student_profile');
        $this->data['sub_page'] = 'inquiry/profile';
        $this->data['main_menu'] = 'student';
        $this->data['headerelements'] = array(
            'css' => array(
                'vendor/dropify/css/dropify.min.css',
    ...
    public function unique_registerid($register) {
        ...
        $this->db->where('inquiry_no', $register);
        $query = $this->db->get('inquiry')->num_rows();
        if ($query == 0) {
            return true;
        } else {
    ...
    public function search() {
        ...
        $this->data['sub_page'] = 'inquiry/search';
        $this->data['main_menu'] = '';
        $this->load->view('layout/index', $this->data);
    }
    ...
    public function transfer() {
        ...
        $this->data['sub_page'] = 'inquiry/transfer';
