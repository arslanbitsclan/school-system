...
class Application_model extends CI_Model {
    ...
    public function getInquiries($classID = '', $sectionID = '', $branchID = '', $deactivate = false, $rollOrder = false) {



        $this->db->select('i.*, c.name as class_name,se.name as section_name,b.name as branch_name,sc.name as category_name, sy.school_year');

        $this->db->from('inquiry as i');

        

        $this->db->join('branch as b', 'i.branch_id = b.id', 'left');

        $this->db->join('class as c', 'i.class_id = c.id', 'left');

        $this->db->join('section as se', 'i.section_id = se.id', 'left');

        $this->db->join('student_category as sc', 'i.category_id = sc.id', 'left');

        $this->db->join('schoolyear as sy', 'i.year_id = sy.id', 'left');



        if ($branchID != '') {

            $this->db->where('i.branch_id', $this->db->escape($branchID));

        }

        if ($classID != '') {

            $this->db->where('i.class_id', $this->db->escape($classID));

        }

        if ($sectionID != '') {

            $this->db->where('i.section_id', $this->db->escape($sectionID));
